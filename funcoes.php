...
function reduzirStr($str,$quantidade){
  ...
  function cadastrar($nome,$email,$peso,$altura,$imc,$classificacao)
    ...
    function cadastrarNoticia($titulo,$img,$descricao,$categoria)
    {
        if(!$titulo ||!$img || !$descricao || !$categoria){return;}
        $sql = "INSERT INTO `noticia_tb` (`titulo`,`img`,`descricao`,`id_categoria`)
        VALUES(:titulo,:img,:descricao,:categoria)";
    ...
    function verificarCategoriaDuplicada($termo)
  {
    ...
    function verificarEmail($email){
      $pdo = Database::conexao();
      $sql = "SELECT * FROM `registro_tb` WHERE `email` = '$email'";
      $stmt = $pdo->prepare($sql);
      $list = $stmt->execute();
      $list = $stmt->fetchAll(PDO::FETCH_ASSOC);
      return $list;
  ...
  function Alterarsenha($Psenha,$Ssenha,$email){
    if(!$Psenha ||!$Ssenha || !$email){return;}
        if($Psenha = $Ssenha){
        $sql = "UPDATE  `registro_tb` SET `senha` = '$Ssenha' WHERE `registro_tb`.`email` = '$email';
        VALUES(:senha,:email)";
        
        $pdo = Database::conexao();
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':senha', $Ssenha);
        $stmt->bindParam(':email', $email);
        $result = $stmt->execute();
        return ($result)?true:false;
        }
